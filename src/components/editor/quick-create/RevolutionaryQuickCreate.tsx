import {
  ...
  EnergyParticles,
  MagicParticles,
} from '@/components/effects/ParticleSystem';
...
import {
  enhancedCardAnalysisService,
  type EnhancedAnalysisResult,
} from '@/services/ai/enhancedCardAnalysisService';
...
interface CreateState {
  step: CreateStep;
  inputMode: 'upload' | 'camera' | 'voice' | null;
  file: File | null;
...
export const RevolutionaryQuickCreate: React.FC<
  RevolutionaryQuickCreateProps
> = ({ isOpen, onClose, onComplete }) => {
  ...
  const { suggestion, isAnalyzing, applySuggestion } = useSmartStyleSuggestions(
    state.imageUrl,
    state.analysis
  );
  ...
  const processImage = async (file: File, imageUrl: string) => {
    try {
      ...
      const recommendations =
        await predictiveService.getPredictiveRecommendations({
  ...
  const applyVariation = async (variationId: string) => {
    ...
    try {
      ...
      toast.success(
        `✨ ${VARIATIONS.find(v => v.id === variationId)?.label} style applied with AI!`
      );
  ...
  const handleCreateCard = async () => {
    ...
    try {
      ...
      const cardData: CardData = {
        ...
        title:
          state.analysis?.text?.playerName ||
          state.analysis?.subject?.name ||
          'Quick Card',
        description: `AI-enhanced ${state.analysis?.cardType || 'custom'} card`,
        image_url: state.imageUrl!,
        thumbnail_url: state.imageUrl!,
        rarity: (state.analysis?.text?.rarity?.toLowerCase() ||
          'common') as any,
  ...
  const handleTextToCard = async () => {
    if (!state.textPrompt) return;

    setState(prev => ({ ...prev, isProcessing: true }));

    try {
      // Generate image from text prompt if no image uploaded
      if (!state.imageUrl) {
        // This would call an AI image generation service
        toast.info('Generating image from description...');
        // For now, we'll require an image
      ...
      const analysis = await enhancedCardAnalysisService.analyzeWithContext(
        state.imageUrl,
        state.file || undefined,
        state.voiceTranscript
      );

      // Extract sport context from voice interpretation
      if (analysis.voiceInterpretation) {
        setState(prev => ({
          ...prev,
          sportContext: {
            sport: analysis.sport?.detected || 'general',
            stats: analysis.voiceInterpretation.extractedStats,
            suggestedEffects: analysis.voiceInterpretation.suggestedEffects,
          },
        }));
      }
      ...
      setState(prev => ({
        ...
        isProcessing: false,
        step: 'enhance',
        recommendations: analysis.suggestions?.effects || [],
      }));
    } catch (error) {
      console.error('Voice to card failed:', error);
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      reader.onload = async e => {
        ...
        // Use enhanced analysis with voice transcript if available
        const analysis = await enhancedCardAnalysisService.analyzeWithContext(
          imageUrl,
          file,
          state.voiceTranscript
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
                  ...
                  >
                    <StyleVariationButton
                      ...
                      onClick={() => {
                        ...
                        applyStyleVariation(
                          variation.id as 'epic' | 'classic' | 'futuristic'
                        );
              ...
              >
                ...
                {state.isProcessing ? (
                  ...
                  >
                    ...
                    <p className='text-xl text-muted-foreground mb-8'>
                      Your card looks perfect! Ready to add it to your
                      collection?
                    </p>
          ...
          {/* Start over button */}
          {(state.step === 'enhance' || state.step === 'publish') &&
            !state.isProcessing && (
              <motion.button
                onClick={resetCreate}
